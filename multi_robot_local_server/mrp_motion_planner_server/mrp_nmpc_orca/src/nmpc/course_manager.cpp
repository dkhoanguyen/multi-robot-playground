...
void CourseManager::set_course_from_nav_msgs(const std::vector<geometry_msgs::msg::PoseStamped> &path, const double &reference_speed) {
  mpc_course_.clear();

  /*convert nav_msgs::path to mpc_course*/
  mpc_course_.resize(path.size());
  for (size_t i = 0; i < path.size(); i++) {
    mpc_course_.x.at(i) = path.at(i).pose.position.x;
